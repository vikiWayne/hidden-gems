...
import { asRecord, readCoordinates, readEnum, readNumber, readString, readStringArray } from '../utils/validation.js'
import { ValidationError } from '../lib/errors.js'

const markerColors = ['purple', 'orange', 'green', 'gold', 'teal', 'blue', 'red', 'pink'] as const
...
export function parseNearbyQuery(input: unknown): { lat: number; lng: number; alt?: number; userId?: string; filter?: string } {
  ...
  const filter = source.filter == null ? undefined : readEnum(source, 'filter', mapFilters)
...
export function parseCreateMessageBody(input: unknown) {
  ...
  const latitude = readNumber(source, 'latitude', { required: true, min: -90, max: 90 }) as number
  const longitude = readNumber(source, 'longitude', { required: true, min: -180, max: 180 }) as number

  return {
    type: source.type == null ? 'text' : readEnum(source, 'type', messageTypes, 'text'),
    content,
    mediaUrl: readString(source, 'mediaUrl'),
    latitude,
    longitude,
    altitude: readNumber(source, 'altitude'),
    visibility: source.visibility == null ? 'public' : readEnum(source, 'visibility', visibilities, 'public'),
    ...
    markerColor: source.markerColor == null ? undefined : readEnum(source, 'markerColor', markerColors),
  }
}
...
export function parseUpdateMessageBody(input: unknown) {
  ...
  return {
    ...
    visibility: source.visibility == null ? undefined : readEnum(source, 'visibility', visibilities),
    allowedUserIds: source.allowedUserIds == null ? undefined : readStringArray(source, 'allowedUserIds'),
    category: readString(source, 'category'),
    markerColor: source.markerColor == null ? undefined : readEnum(source, 'markerColor', markerColors),
  }
}
...
export function parseCreateChestBody(input: unknown) {
  ...
  return {
    ...
    latitude: readNumber(source, 'latitude', { required: true, min: -90, max: 90 }) as number,
    longitude: readNumber(source, 'longitude', { required: true, min: -180, max: 180 }) as number,
    altitude: readNumber(source, 'altitude'),
    xpReward: readNumber(source, 'xpReward'),
    coinReward: readNumber(source, 'coinReward'),
    variant: source.variant == null ? 'normal' : readEnum(source, 'variant', chestVariants, 'normal'),
    createdBy: readString(source, 'createdBy'),
  }
}
...
export function parseViewportQuery(input: unknown) {
  const source = asRecord(input, 'query')
  const minLat = readNumber(source, 'minLat', { required: true, min: -90, max: 90 }) as number
  const maxLat = readNumber(source, 'maxLat', { required: true, min: -90, max: 90 }) as number
  const minLng = readNumber(source, 'minLng', { required: true, min: -180, max: 180 }) as number
  const maxLng = readNumber(source, 'maxLng', { required: true, min: -180, max: 180 }) as number
  ...
  return {
    ...
    filter: source.filter == null ? undefined : readEnum(source, 'filter', mapFilters),
  }
}

export function parseCreateLootBody(input: unknown) {
  const source = asRecord(input, 'body')
  const type = source.type == null ? 'diamond' : readEnum(source, 'type', lootTypes, 'diamond')
  return {
    type,
    content: readString(source, 'content', { required: true }) as string,
    latitude: readNumber(source, 'latitude', { required: true, min: -90, max: 90 }) as number,
...
export function parseSearchUsersQuery(input: unknown): { q: string; limit: number } {
  const source = asRecord(input, 'query')
...
export function parseRateItemBody(input: unknown): { userId: string; itemType: 'message' | 'chest' | 'loot'; itemId: string; rating: number } {
  const source = asRecord(input, 'body')
  return {
    ...
    rating: readNumber(source, 'rating', { required: true, min: 1, max: 5 }) as number,
  }
}

export function parseRatingParams(input: unknown): { itemType: 'message' | 'chest' | 'loot'; itemId: string } {
  const source = asRecord(input, 'params')
  return {
    itemType: readEnum(source, 'itemType', itemTypes),
    itemId: readString(source, 'itemId', { required: true }) as string,
