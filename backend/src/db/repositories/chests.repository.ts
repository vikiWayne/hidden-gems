...
export function getChestById(id: string): Chest | undefined {
  const row = getDb().prepare(`SELECT * FROM chests WHERE id = ?`).get(id) as
    | ChestRow
    | undefined;
...
export function getChestsInViewport(
  ...
  const rows = getDb()
    ...
    .all(minLat, maxLat, minLng, maxLng) as ChestRow[];
...
export function getChestsByCreator(userId: string): Chest[] {
  const rows = getDb()
    .prepare(
      `SELECT * FROM chests WHERE created_by = ? ORDER BY created_at DESC`,
    )
    .all(userId) as ChestRow[];
