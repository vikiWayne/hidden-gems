...
export function ExplorePage() {
  ...
  return (
    <div className="space-y-4 pb-20">
      {locationError ? (
        <Banner title="Location unavailable" severity="error">
          <p className="text-xs text-[var(--color-text-muted)]">
            {locationError}. Map is still available to explore.
          </p>
        </Banner>
      ) : isLocationLoading || !userLocation ? (
        <div className="flex items-center gap-2 rounded-2xl border-2 border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-4 py-3">
          <Spinner size="sm" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        >
          ...
          <div className="absolute top-2 left-2 z-[1000] flex flex-col gap-2">
            ...
            {filters.map((f) => (
              <button
                key={f.value}
                onClick={() => startTransition(() => setMapFilter(f.value))}
        ...
        <div className="flex flex-col gap-4 order-2">
          ...
          >
            <h3 className="text-xl font-black uppercase tracking-tight mb-2 text-[var(--color-text-primary)]">
            ...
            <button
              ...
              className={`w-full py-3 rounded-2xl font-black uppercase tracking-widest text-white transition-all active:translate-y-1 ${
                chestHunterMode
